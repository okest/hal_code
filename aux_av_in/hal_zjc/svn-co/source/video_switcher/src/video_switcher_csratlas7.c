...
#if 0
static char cvbs_read(unsigned int u32ReadLen)
{
	char cRet 	= -1;
	cvbs_Param_t cvbs_ReadParam;
	cvbs_ReadParam.bFlag 		= 1;
	cvbs_ReadParam.u32Len 		= u32ReadLen;

	cvbs_ReadParam.u8Data = (unsigned char *)malloc(sizeof(unsigned int) * (cvbs_ReadParam.u32Len + 1));
	if(cvbs_ReadParam.u8Data == NULL)
	{
        #if HAL_VIDEO_SWITCHER_DEBUG_EN == 1
		printf("malloc() in cvbs_read() failed\n");
        #endif
		return -1;
	}

	cRet= read( g_iCvbs_fd, &cvbs_ReadParam, sizeof(cvbs_ReadParam) );
    if(cRet < 0)
    {
        #if HAL_VIDEO_SWITCHER_DEBUG_EN == 1
        printf("read() in cvbs_read() error\n");
        #endif
        return -1;
    }

    #if HAL_VIDEO_SWITCHER_DEBUG_EN == 1
    printf("\n[%s][%d]  [ data0 = 0x%x  data1 =0x%x]   \n ", __func__,__LINE__,cvbs_ReadParam.u8Data[0],cvbs_ReadParam.u8Data[1],cvbs_ReadParam.u8Data[2] );
    #endif
...
static int cvbs_write(unsigned char *data,unsigned int u32WriteLen)
{
	char cRet = -1;
...
int open_video_channel(void)
{
	...
	if(ret != 0)
	{
		printf("[%s:%d]open_fms6502() in open_video_channel() failed\n", __FILE__, __LINE__);
		return -1;
	}
...
int close_video_channel(void)
{
	...
	if(ret != 0)
	{
		printf("[%s:%d]close_fms6502() in close_video_channel() failed\n", __FILE__, __LINE__);
		return -1;
	}
...
int createVideoSwitcherHal(T_HalVideoSwitcherDevice** device)
{
	...
	dev->selectVideoSource = selectVideoSource;
	dev->readRvcGpioData = readRvcGpioData;
