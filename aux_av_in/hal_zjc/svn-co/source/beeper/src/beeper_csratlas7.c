...
#define DEVICE_NAME "/dev/atlas7_beeper" /* 设备名称 */
...
/********************************************************************
  ...
  返回值:    大于等于0:成功; 小于0:失败
********************************************************************/
static int beeperSet(unsigned int value,unsigned int msecs)
{
        beeper_param_t beeper_param;
        beeper_param.pwm_value = value;
        beeper_param.delay_ms = msecs;

        return write(fd, &beeper_param, sizeof(struct beeper_param));
}
